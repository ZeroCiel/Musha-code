...
inst.guard_on = true
inst.count_w = inst.count_w * 0--扣除音乐值
...
inst.components.talker:Say(string..MUSHA_TALK_WILSON_RANDOM..tostring(math.random(1,6)))
end
...
inst.components.talker:Say(string..MUSHA_TALK_WILSON_MUSHA..tostring(math.random(1,6)))
...
if inst.moondrake_on and not (inst.sg:HasStateTag("rowing") or inst.sg:HasStateTag("sailing") or inst.sg:HasStateTag("sail") or inst.sg:HasStateTag("row") or inst.sg:HasStateTag("boating") or inst:HasTag("aquatic") or inst:HasTag("ready_to_snap")) then
local drake = SpawnPrefab("moonnutdrake2")
local drakeangle = math.random(1, 360)
local x,y,z = inst.Transform:GetWorldPosition()
	drake.Transform:SetPosition(x, y + math.random(2,2) * 2 or y, z)
...
local x,y,z = inst.Transform:GetWorldPosition()
	spore.Transform:SetPosition(x, y + math.random() * 2 or y, z)
--end
end	
end
...
-- inst.treasure = inst.treasure + TUNING.Musha_Sniff
-- end

if not inst.No_Sleep_Princess then
if not inst.sleep_on and not inst.tiny_sleep and not inst:HasTag("playerghost") then
if TheWorld.state.isdusk and inst.components.stamina_sleep.current >0 then
...
if not inst.tiny_sleep and not inst.sleep_on then
	...
	inst.components.talker:Say(string..MUSHA_TALK_SLEEP_DIZZY..tostring(math.random(1,7)))
...
inst.sg:GoToState("wakeup")
inst.musha_press = false
inst.components.talker:Say(string..MUSHA_TALK_SLEEP_NEED_LIGHT..tostring(math.random(1,5)))
...
inst.sg:GoToState("wakeup")
inst.musha_press = false
inst.components.talker:Say(string..MUSHA_TALK_SLEEP_DANGER..tostring(math.random(1,5)))
end 
end 
end
...
inst.components.fatigue_sleep:DoDelta(TUNING.Musha_Tired_valkyrie)	
end	
...
-- passive skills
-- critical hit
local function on_Critical_1(inst, data)--X技能
...
local function on_Lshield_1(inst, attacked)--护盾技能
   local Lshield1 = 0.2
...
function Call_lightining_on(inst)--闪电技能 根据等级设置变化
...
elseif inst.level >= 1880 and inst.level < 5000 then
	inst.SoundEmitter:PlaySound("dontstarve/rain/thunder_close")
	if inst.loud_2 or inst.loud_3 then
...
local result = math.floor((value-10)/10)+2
return result
elseif value <= 130 then
local result = math.floor((value-50)/20)+6
return result
elseif value <= 380 then
local result = math.floor((value-130)/50)+10
return result
elseif value <= 880 then
local result = math.floor((value-380)/100)+15
return result
elseif value <= 2130 then
local result = math.floor((value-880)/250)+20
return result
elseif value <= 4030 then
local result = math.floor((value-2130)/475)+25
return result
elseif value <= 5000 then
local result = math.floor((value-4030)/970)+29
return result
end
end

--告诉玩家每一级升级所需的经验算法
local function LevelInfo(value)
if value <= 10 then
...
local Musha_health = TUNING.Musha_health
local Musha_exp = Level(inst.level)
local Levelup_exp = LevelInfo(inst.level)

if inst.level < 5000 then
inst.components.health.maxhealth = math.ceil(Musha_exp * Musha_health)+80
inst.components.sanity.max = math.ceil(Musha_exp * Musha_health)+80
inst.components.talker:Say(STRINGS.MUSHA_LEVEL_EXP.."\n".. (inst.level).."/".. (Levelup_exp))
...
inst.components.combat.damagemultiplier = TUNING.Musha_damage_Melee
...
inst.components.combat.damagemultiplier = TUNING.Musha_damage_range
inst.damage_set_melee = true	
elseif inst.x2damage and not inst.damage_set2_melee then
inst.components.combat.damagemultiplier = TUNING.Musha_damage_range * 2
...
if inst.charging_music and inst.music < 100 then
inst.music = inst.music + TUNING.Musha_Music
fullcharged_music(inst)
end	
end)
...
if v:IsValid() and v.entity:IsVisible() and v.components.health and not v.components.health:IsDead() and not (v:HasTag("berrythief") or v:HasTag("bird") or v:HasTag("butterfly")) and not v:HasTag("groundspike") and not v:HasTag("player") and not v:HasTag("companion") and not v:HasTag("stalkerminion") and not v:HasTag("structure") and v.components.combat ~= nil and (v.components.combat.target == inst or v:HasTag("monster") or v:HasTag("burn")) then
	...
	if inst.lightaura then
		...
		if inst.components.spellpower ~= nil and inst.components.spellpower.current <100 then
			...
			inst.components.spellpower:DoDelta(TUNING.Musha_ManaData)
