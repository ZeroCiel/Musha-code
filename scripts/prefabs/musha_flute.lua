...
local user = musician
	...
	if inst.components.health and (inst:HasTag("player") or inst:HasTag("follower")) then
	...
	if inst.components.health and (inst:HasTag("companion") or inst:HasTag("yamacheb")) then
   	SpawnPrefab("green_leaves").Transform:SetPosition(inst:GetPosition():Get())	
