...
local function splash_on(inst)		
	...
	if owner ~= nil and (owner.sg:HasStateTag("rowing") or owner.sg:HasStateTag("sailing") or owner.sg:HasStateTag("sail") or owner.sg:HasStateTag("row") or owner.sg:HasStateTag("boating") or owner:HasTag("aquatic")) or owner.sg:HasStateTag("boat") and 
...
--or inst.sg:HasStateTag("attack") or inst.sg:HasStateTag("hit")
	if inst.sg:HasStateTag("idle") or
	inst.sg:HasStateTag("idle_blink") or
	inst.sg:HasStateTag("taunt") or
	inst.sg:HasStateTag("open") or
	inst.sg:HasStateTag("close") or
	inst.sg:HasStateTag("hit") or
	inst.sg:HasStateTag("eat") or
	inst.sg:HasStateTag("attack2") or
	inst.sg:HasStateTag("sleep") or
	inst.sg:HasStateTag("idle_peep") or
	inst.sg:HasStateTag("cry") or
	inst.sg:HasStateTag("mine") or
	inst.sg:HasStateTag("chop") or
	inst.sg:HasStateTag("throw") or
	inst.sg:HasStateTag("action") or
	inst.sg:HasStateTag("walk_stop") or
	inst.sg:HasStateTag("run_stop") or
	inst.sg:HasStateTag("animover") then
...
-- local x,y,z = inst.Transform:GetWorldPosition()
-- local leader = inst.ownerlist and GetTheWorldPlayerById(inst.ownerlist.master)
-- local ents = TheSim:FindEntities(x,y,z, 10, {"leader"})
	-- if leader ~= nil then
	-- for k,v in pairs(ents) do
-- --	if v.components.leader:CountFollowers("yamcheb") == 0 then
		-- inst.components.follower:SetLeader(v)
...
inst:ListenForEvent( "healthdelta", updatestats)
	...
	inst.level = 0
    --inst:ListenForEvent("entity_death", function(wrld, data) onkilll(inst, data) end, TheWorld)
...
local function create_smallbird_super()
	...
	-----inst.Light:Enable(true)
-----------------------
--The BUG!They all thought the bird had a container, but it didn't!
